...
uint8_t AMS_AS5048B::readReg8(uint8_t address) {
	...
	Wire2.beginTransmission(_chipAddress);
	Wire2.write(address);
	requestResult = Wire2.endTransmission(false);
	if (requestResult){
		Serial.print("I2C error: ");
		Serial.println(requestResult);
	}

	Wire2.requestFrom(_chipAddress, nbByte2Read);
	readValue = (uint8_t) Wire2.read();
...
uint16_t AMS_AS5048B::readReg16(uint8_t address) {
    ...
    requestResult = Wire2.requestFrom((uint8_t) _chipAddress, nbByte2Read, address, 1, false);
    if (requestResult == 0) {
        Serial.print("I2C error: ");
        Serial.println(requestResult);
